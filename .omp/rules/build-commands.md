---
description: Build, test, lint, and dev commands using Bun and Turborepo
globs:
  - "package.json"
  - "turbo.json"
  - "biome.json"
...
| `turbo run build` | Build all packages (@muir/base → @muir/extra → @muir/form → @muir/capacitor → muir-docs) |
...
| Command | Description |
|---|---|
| `turbo run build --filter=@muir/base` | Build only the @muir/base library |
...
| `bun run lint` | Biome check and tsc |

### @muir/extra (`packages/extra/`)
...
### @muir/capacitor (`packages/capacitor/`)
...
### @muir/navigation (`packages/navigation/`)
