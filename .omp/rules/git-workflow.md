---
description: Trunk-based branch strategy and Conventional Commits format with muir-specific scopes
globs:
  - ".git/**"
---
...
Use the package or subsystem name: `base`, `extra`, `form`, `capacitor`, `navigation`, `docs`, `storybook`, `biome`, `ci`, `deps`, etc.

In a monorepo, scope helps identify which package is affected. Use `base` for @muir/base, `extra` for @muir/extra, `form` for @muir/form, `capacitor` for @muir/capacitor, `navigation` for @muir/navigation, and `docs` for the documentation app.

### Examples

```
feat(base/tooltip): add controlled/uncontrolled modes
fix(base/button): resolve focus ring missing in Safari
test(base/modal): add rstest snapshot for open/close states
chore(deps): bump biome to 1.9.0
ci: add Storybook publish step on merge to main
docs(switch): add form integration example
fix(capacitor): fix missing re-export for FormSlider
```
