...
Use the package or subsystem name: `muir`, `muir-capacitor`, `docs`, `button`, `tokens`, `storybook`, `biome`, `ci`, etc.

In a monorepo, scope helps identify which package is affected. Use `muir` for the core library, `muir-capacitor` for the Capacitor wrapper, and `docs` for the documentation app.

### Examples

```
feat(muir/tooltip): add controlled/uncontrolled modes
fix(muir/button): resolve focus ring missing in Safari
test(muir/modal): add rstest snapshot for open/close states
chore(deps): bump biome to 1.9.0
ci: add Storybook publish step on merge to main
docs(switch): add form integration example
fix(muir-capacitor): fix missing re-export for FormSlider
```
