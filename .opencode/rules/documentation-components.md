# Component Documentation Rules (`.mdx`)
...
- `source` - required field; path to the component source directory (relative to `packages/muir/`)
...
import { Text } from 'muir-capacitor/basic';
```

- Use `muir-capacitor` package imports for all live demos
- Import every component used in live demos
- **Never** import from `muir` directly in the docs app — the docs app depends on `muir-capacitor`
...
import { Text } from 'muir/basic';

<Text type="label"><Switch /> Enable notifications</Text>
\`\`\`
```

- Use the **published package name** (`muir`, `muir/basic`, `muir/extra`) in usage code blocks — these are what consumers will write
- Show import + render, nothing else
- If the component belongs to a specific export group, use that group:
  - Basic components: `import { Switch } from 'muir/basic'`
  - Form wrappers: `import { Form, FormCheckbox } from 'muir/form'`
  - Extra components: `import { ToggleButton } from 'muir/extra'`
  - General: `import { Button } from 'muir'`

Note: usage code blocks show `muir` imports (the published package name), while the live demo import block at the top of the `.mdx` file uses `muir-capacitor` (the docs app dependency).
...
- Reference `.opencode/rules/tools.md` for the division of labor
